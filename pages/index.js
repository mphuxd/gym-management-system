  ...
  return (
    data && (
      <ScrollArea.Root className='relative overflow-hidden rounded h-40 hover:cursor-pointer '>
        ...
        <ScrollArea.Scrollbar className='flex px-[1px] w-2 bg-gray-200' orientation='vertical'>
          <ScrollArea.Thumb className='flex-grow w-1 rounded-lg bg-gray-400 relative' />
        </ScrollArea.Scrollbar>
        <ScrollArea.Corner className='bg-black' />

        <DashboardNotesDialog open={open} setOpen={setOpen} note={data.notes} refetch={refetch} />
      </ScrollArea.Root>
    )
  );
}
